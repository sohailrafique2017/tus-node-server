...
class S3Store extends DataStore {
    ...
    _bucketExists() {
        ...
        return this.client
            ...
            .then((data) => {
                if (!data) {
                    throw new Error(
                        `bucket "${this.bucket_name}" does not exist`
                    )
                }
    ...
    _processUpload(metadata, req, current_part_number) {
        return this._uploadPart(metadata, req, current_part_number)
            ...
            .then((current_offset) => {
                if (
                    parseInt(metadata.file.upload_length, 10) ===
                    current_offset.size
                ) {
    ...
    async create(req) {
        ...
        try {
            ...
            console.warn(
                '[S3Store] create: check your `namingFunction`. Error',
                err
            )
                ...
                .then((results) => {
                    ...
                    return this._processUpload(
                        ...
                        .catch((err) => {
                            ...
                            if (
                                ['RequestTimeout', 'NoSuchUpload'].includes(
                                    err.code
                                ) &&
                                isClosed
                            ) {
