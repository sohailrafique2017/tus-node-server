...
const { encryptData } = require('../../utils/helper-functions')
...
class S3Store extends DataStore {
  constructor(options) {
    ...
    assert.ok(
      ...
      '[S3Store] `secretAccessKey` must be set'
    )
   ...
   */
  _bucketExists() {
    console.log('inside bucket check')
    return this.client
      ...
      .catch((err) => {
        if (err.statusCode === 404) {
          log(`bucket "${this.bucket_name}" not exists`)
          log('creating new bucket for user')
          this.client.createBucket(
            ...
            (err, data) => {
              if (err) {
                throw new Error(
                  `[S3Store] some thing went wrong, cannot create bucket "${this.bucket_name}" `
                )
              }
              if (!data) {
                throw new Error(
              ...
              log(`bucket "${this.bucket_name}" created`)
  ...
  /**
     * process the chunk
     *
     * @param  {object} stream_obj http.incomingMessage
     * @param  {string} total_length   total file's length
     * @param  {integer} offset     starting offset/part number
     * @param  {string} file_id   file's unique id
     * @return {Promise} preprocessed chunk (data)
     */
  _processChunk(stream_obj, offset, total_length, file_id) {
    return new Promise((resolve, reject) => {
      let data = Buffer.from([])

      const old_offset = 0
      let new_offset = stream_obj.partSize;
      stream_obj.on('data', (buffer) => {
        if (!this.currentChunk) {
          data = Buffer.concat([data, buffer])
          if (data.length >= new_offset || (offset > parseInt(total_length / stream_obj.partSize) && ((offset - 1) * stream_obj.partSize) + data.length === +total_length)) {
            offset > parseInt(total_length / stream_obj.partSize) ? this.currentChunk = data.slice(old_offset) : this.currentChunk = data.slice(old_offset, new_offset)
            console.log('current chunk\'s length: ', this.currentChunk.length);
            const partsInfo = JSON.parse((fs.readFileSync(`./cache/${this.bucket_name + file_id}`)).toString())
            partsInfo[`${offset}`] = this.currentChunk.length
            fs.writeFileSync(`./cache/${this.bucket_name + file_id}`, JSON.stringify(partsInfo))
            console.log('chunk size before encryption: ', this.currentChunk.length);
            this.currentChunk = this._encryptData(this.currentChunk)
            resolve(this.currentChunk)
          }
        }
      });
    });
   ...
   */
  async _uploadPart(metadata, read_stream, current_part_number, file_id) {
    // const data = await this._processChunk(read_stream, current_part_number, metadata.file.upload_length, file_id)
    // console.log('data after processing: ', data.length);
    ...
    return this.client
      .uploadPart({
        ...
        Body: read_stream // data
      })
      .promise()
      .then(async (data) => {

        console.log(
          `[${metadata.file.id}] finished uploading part #${current_part_number}`
        )
        log(
          `[${metadata.file.id}] finished uploading part #${current_part_number}`
        )
        ...
        // updating bandwidth
        this._updateBandwidth(read_stream.length, read_stream.headers.authorization, read_stream.headers.encryption_public_key)
   ...
   * @memberof S3Store
   */
  _processUpload(metadata, req, current_part_number, file_id) {
    return this._uploadPart(metadata, req, current_part_number, file_id, req.bucket_name)
        ...
        if (parseInt(metadata.file.upload_length, 10) === current_offset.size) {
          return this._finishMultipartUpload(metadata, current_offset.parts)
            .then(async (location) => {
              ...
              console.log(
                `[${metadata.file.id}] finished uploading: ${location}`
              )
              // metadata: {
              // 	type: req.headers.type,
              // 	dir_type: req.headers.dir_type,
                ...
                const response = await axios.default.post(
                  process.env.FILE_UPLOAD_URL,
                  {
                    data: await encryptData({
                      ...
                      parent_id: +req.headers.parent_id,
                      name_on_cloud: req.fileNameOnBucket,
                      size: +metadata.file.upload_length,
                      path: req.headers.path,
                      phrase_enc_key: req.headers.phrase_enc_key,
                      pass_enc_key: req.headers.pass_enc_key,
                      thumbnail_metadeta: req.headers.thumbnail_metadeta,
                      is_internall_call: true,
                      file_enc_key: req.headers.file_enc_key
                    }, req.headers.encryption_public_key)
                  },
                  {
                    headers: {
              ...
              fs.unlink(`./cache/${this.bucket_name + file_id}`, () => { })
  ...
  _retrieveParts(file_id, part_number_marker) {
    ...
    return this.client
      ...
      .then((data) => {
        ...
        // overwriting the original parts size
        ...
        if (data.Parts.length) {
          ...
          for (const part of data.Parts) {
            part.Size = partsInfo[part.PartNumber]
          }
        }
   ...
   */
  _countParts(file_id) {
    return this._retrieveParts(file_id).then((parts) => parts.length)
  }
  ...
  async create(req) {
    ...
    try {
      file_id = await this.generateFileName(req)
    } catch (err) {
  ...
  write(req, file_id, offset) {
    if (offset === 0) {
      fs.writeFileSync(`./cache/${this.bucket_name + file_id}`, '{}')

    }
    return new Promise((resolve, reject) => {
      Promise.all([this._countParts(file_id), this._getMetadata(file_id)])
        .then((results) => {
          ...
          return this._processUpload(metadata, req, current_part_number, file_id)
            .then(resolve)
            .catch((err) => {
              ...
              if (
                ['RequestTimeout', 'NoSuchUpload'].includes(err.code) &&
                isClosed
              ) {
                if (err.code === 'RequestTimeout') {
                  console.warn(
                    '[S3Store] Request "close" event was emitted,' +
  ...
  async _updateBandwidth(bytes, jwt, encryption_public_key) {
    try {
      axios.default.post(process.env.UPDATE_BANDWIDTH_URL,
        {
          data: await encryptData({
            bytes: bytes,
            is_internall_call: true
      ...
      console.log('error in _updateBandwidth: ', error);
    }
    return
  }
}
