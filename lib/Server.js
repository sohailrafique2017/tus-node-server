...
class TusServer extends EventEmitter {
  ...
  async handle(req, res) {
    ...
    if (req.method === 'POST') {
      ...
      try {
        const response = await axios.default.get(process.env.AVAILABLE_STORAGE, {
          headers: {
            authorization: req.headers.authorization,
          },
        })
        const avaibaleStorage = response.data.response.freeStorage
      ...
      try {
        const response = await axios.default.get(process.env.AVAILABLE_BANDWIDTH, {
          headers: {
            authorization: req.headers.authorization,
          },
        })
        const avaibaleBandwidth = response.data.response.freeBandwidth
