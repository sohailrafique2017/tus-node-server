...
const { encryptData, decryptData } = require('../utils/helper-functions')
...
class TusServer extends EventEmitter {
  ...
  async handle(req, res) {
    ...
    if (req.method === 'POST') {
      ...
      if (!req.headers.encryption_public_key) {
        res.writeHead(409, {})
        res.write('encryption_public_key is required\n')
        return res.end()
      }

      if (!req.headers.type || (req.headers.type !== 'FILE' && req.headers.type !== 'THUMBNAIL')) {
        res.writeHead(403, {})
        res.write(
          'Document type is missing, it should be FILE or THUMBNAIL for file upload\n'
        )
        return res.end()
      }

      if (req.headers.type === 'THUMBNAIL') {
        const thumbnailMetadeta = JSON.parse(req.headers.thumbnail_metadeta)
        if (!thumbnailMetadeta.hasOwnProperty('parent_id') || !thumbnailMetadeta.hasOwnProperty('filename')) {
          res.writeHead(403, {})
          res.write(
            'Invalid metadeta for thumbnail\n'
          )
          return res.end()
        }
      ...
      if (!req.headers.pass_enc_key || req.headers.pass_enc_key === '') {
        res.writeHead(403, {})
        res.write('pass_enc_key is missing\n')
        return res.end()
      }

      if (!req.headers.phrase_enc_key || req.headers.phrase_enc_key === '') {
        res.writeHead(403, {})
        res.write('phrase_enc_key is missing\n')
        return res.end()
      }
      ...
      try {
        let response = await axios.default.get(process.env.AVAILABLE_STORAGE, {
          headers: {
            authorization: req.headers.authorization,
          },
        })
        response = await decryptData(response.data.response)
        const avaibaleStorage = response.freeStorage
        if (avaibaleStorage < +req.headers['upload-length']) {
          res.writeHead(406, {})
          res.write(`Your available storage ${avaibaleStorage} is less then your file size ${req.headers['upload-length']}\n`)
          return res.end()
        }
      ...
      try {
        let response = await axios.default.get(process.env.AVAILABLE_BANDWIDTH, {
          headers: {
            authorization: req.headers.authorization,
          },
        })
        response = await decryptData(response.data.response)
        const avaibaleBandwidth = response.freeBandwidth
        if (avaibaleBandwidth < +req.headers['upload-length']) {
          res.writeHead(406, {})
          res.write(`Your are out of bandwidth\n`)
          return res.end()
        }
      ...
      try {
        let response = await axios.default.get(
          process.env.GET_SHARED_STORAGES_IDS,
          {
            headers: {
              authorization: req.headers.authorization,
            },
        ...
        response = await decryptData(response.data.response)
        if (response.includes(+req.headers.parent_id)) {
          if (!req.headers.file_enc_key) {
            res.writeHead(400, {})
            res.write(`file_enc_key key is required\n`)
            return res.end()
          }
        }
      } catch (error) {
    ...
    if (req.method !== 'OPTIONS') {
      if (!req.fileNameOnBucket) {
        ...
        fileNameOnBucket += req.headers.parent_id + '.' + fileNameInfo[fileNameInfo.length - 1]
        req['fileNameOnBucket'] = createHmac('sha256', '').update(fileNameOnBucket).digest('hex')
