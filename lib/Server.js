...
class TusServer extends EventEmitter {
    ...
    async handle(req, res) {
        // TODO: have to check validation in POST req only
        ...
        if (!req.headers.dir_type || req.headers.dir_type !== 'SUB_DIR') {
            res.writeHead(403, {})
            res.write(
                'Dir type is missing, for file upload it should be SUB_DIR\n'
            )
            return res.end()
        }
        ...
        if (requestLogging.length > 0) {
            ...
            if (bucketInstance) {
                ...
                // this._datastore.bucket_name = bucketInstance.bucket_name
            } else
                requestLogging.push({
                    id: req.bucket_name,
                    bucket_name: req.bucket_name,
                })
            // this._datastore.bucket_name = req.headers['bucket_name']
            this.datastore = new S3Store({
                path: '/',
                endpoint: ep,
